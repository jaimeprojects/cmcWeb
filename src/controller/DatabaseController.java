...
public ArrayList<String> getSavedByUser(String username){
	  ArrayList<String> savedByUser =  new ArrayList<String>();
	  String[][] savedSchools = dblib.user_getUsernamesWithSavedSchools();
	  for(int i = 0; i < savedSchools.length; i++ )
		  if(savedSchools[i].equals(username)) {
			  for(int o = 0; o < savedSchools[i].length; o++) {
				  System.out.println(savedSchools[i].length);
				  System.out.println(savedSchools[i][o]);
				  savedByUser.add(savedSchools[i][o]);
			  }
		  }
